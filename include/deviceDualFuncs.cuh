...
template <typename T>
__global__ void dual_pow(DualNumber<T>* input, DualNumber<T>* output, int numElements,  T exponent) {
...
template <typename T>
__global__ void device_dual_add(const DualNumber<T>* a, const DualNumber<T>* b, DualNumber<T>* result, int numElements) {
    int idx = threadIdx.x + blockIdx.x * blockDim.x;
    if (idx < numElements) {
...
template <typename T>
__global__ void dual_sub(const DualNumber<T>* a, const DualNumber<T>* b, DualNumber<T>* result, int numElements) {
    int idx = threadIdx.x + blockIdx.x * blockDim.x;
    if (idx < numElements) {
        result[idx].real = a[idx].real - b[idx].real;
        result[idx].dual = a[idx].dual - b[idx].dual;
...
template <typename T>
__global__ void dual_mul(const DualNumber<T>* a, const DualNumber<T>* b, DualNumber<T>* result, int numElements) {
    int idx = threadIdx.x + blockIdx.x * blockDim.x;
    if (idx < numElements) {
        result[idx].real = a[idx].real * b[idx].real;
...
template <typename T>
__global__ void dual_div(const DualNumber<T>* a, const DualNumber<T>* b, DualNumber<T>* result, int numElements) {
    int idx = threadIdx.x + blockIdx.x * blockDim.x;
    if (idx < numElements) {
...
template <typename T>
__global__ void dual_sin(const DualNumber<T>* input, DualNumber<T>* output, int numElements) {
    int idx = threadIdx.x + blockIdx.x * blockDim.x;
    if (idx < numElements) {
...
template <typename T>
__global__ void dual_cos(const DualNumber<T>* input, DualNumber<T>* output, int numElements) {
    int idx = threadIdx.x + blockIdx.x * blockDim.x;
    if (idx < numElements) {
...
template <typename T>
__global__ void dual_tan(const DualNumber<T>* input, DualNumber<T>* output, int numElements) {
    int idx = threadIdx.x + blockIdx.x * blockDim.x;
    if (idx < numElements) {
...
template <typename T>
__global__ void dual_acos(const DualNumber<T>* input, DualNumber<T>* output, int numElements) {
    int idx = threadIdx.x + blockIdx.x * blockDim.x;
    if (idx < numElements) {
...
template <typename T>
__global__ void dual_asin(const DualNumber<T>* input, DualNumber<T>* output, int numElements) {
    int idx = threadIdx.x + blockIdx.x * blockDim.x;
    if (idx < numElements) {
        T one_minus_x_squared = 1.0 - input[idx].real * input[idx].real;
        T sqrt_term = sqrt(one_minus_x_squared);
...
template <typename T>
__global__ void dual_atan(const DualNumber<T>* input, DualNumber<T>* output, int numElements) {
    int idx = threadIdx.x + blockIdx.x * blockDim.x;
    if (idx < numElements) {
